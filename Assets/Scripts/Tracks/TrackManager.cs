...
public class TrackManager : MonoBehaviour
{
    ...
    public IEnumerator Begin()
    {
        if (!m_Rerun)
        {
            ...
            // Spawn the player
            var op = Addressables.InstantiateAsync(Progress.instance.playerInfo.characters[Progress.instance.playerInfo.usedCharacter],
                Vector3.zero,
                Quaternion.identity);
            yield return op;
            if (op.Result == null || !(op.Result is GameObject))
            {
                Debug.LogWarning(string.Format("Unable to load character {0}.", Progress.instance.playerInfo.characters[Progress.instance.playerInfo.usedCharacter]));
                yield break;
            }
            Character player = op.Result.GetComponent<Character>();

            player.SetupAccesory(Progress.instance.playerInfo.usedAccessory);
            ...
            //Instantiate(CharacterDatabase.GetCharacter(Progress.instance.characters[Progress.instance.usedCharacter]), Vector3.zero, Quaternion.identity);
            ...
            if (m_IsTutorial)
                ...
                m_CurrentThemeData = ThemeDatabase.GetThemeData(Progress.instance.playerInfo.themes[Progress.instance.playerInfo.usedTheme]);
            ...
            Progress.instance.StartRunMissions(this);
        }
    ...
    public void End()
    {
        ...
        {
            Progress.instance.Add(characterController.inventory.GetConsumableType());
    ...
    void Update()
    {
        ...
        if (!m_IsTutorial)
        {
            ...
            int currentTarget = (Progress.instance.playerInfo.rank + 1) * 300;
            if (m_TotalWorldDistance > currentTarget)
            {
                Progress.instance.playerInfo.rank += 1;
                Progress.instance.Save();

            }

            Progress.instance.UpdateMissions(this);
        }
            ...
            while (currentWorldPos < segment.worldLength)
            {
                ...
                if (laneValid)
                {
                    ...
                    else if (Random.value < premiumChance)
                    {
                        ...
                        //    yield break;
                        //}
                        GameObject premiumObject = Instantiate(currentTheme.premiumCollectible, pos, rot);
                        //toUse = op.Result as GameObject;
                        toUse = premiumObject;
                        toUse.transform.SetParent(segment.transform, true);
                    }
