	...
	: MonoBehaviour
{
	...
	public void Populate()
	{
		...
		if (displayPlayer)
		{
			place = Progress.instance.GetScorePlace(int.Parse(playerEntry.score.text));
		...
		if (!forcePlayerDisplay || Progress.instance.playerInfo.highscores.Count < entriesCount)
		...
		for (int i = 0; i < entriesCount; ++i)
		{
		    ...
		    if (Progress.instance.playerInfo.highscores.Count > currentHighScore)
		    {
		        hs.gameObject.SetActive(true);
		        hs.playerName.text = Progress.instance.playerInfo.highscores[currentHighScore].name;
		        hs.number.text = (localStart + i + 1).ToString();
		        hs.score.text = Progress.instance.playerInfo.highscores[currentHighScore].score.ToString();
