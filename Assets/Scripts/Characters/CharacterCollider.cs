    ...
    protected void OnTriggerEnter(Collider c)
    {
        if (c.gameObject.layer == k_CoinsLayerIndex)
		{
			...
			if (c.GetComponent<Coin>().isPremium)
            {
                ...
                Progress.instance.playerInfo.premium += 1;
            ...
            else
            {
                ...
                Progress.instance.playerInfo.coins += 1;
				controller.coins += 1;
        ...
        else if(c.gameObject.layer == k_ObstacleLayerIndex)
        {
			...
			else
			{
				...
				if (controller.trackManager.score > Progress.instance.playerInfo.highScore)
				{
					Progress.instance.playerInfo.highScore = controller.trackManager.score;
					Progress.instance.Save();

					#if !UNITY_EDITOR
							SetToLeaderboard(Progress.instance.playerInfo.highScore); 
					#endif
                }
